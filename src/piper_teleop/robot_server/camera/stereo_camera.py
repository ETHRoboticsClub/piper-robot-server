...
def crop_stereo_image(frame, frame_height, frame_width, edge_crop):
    mid = frame.shape[1] // 2
    ...
    if frame_rgb_left.shape[:2] != (frame_height, frame_width):
        frame_rgb_left = cv2.resize(frame_rgb_left, (frame_width, frame_height))
    if frame_rgb_right.shape[:2] != (frame_height, frame_width):
        frame_rgb_right = cv2.resize(frame_rgb_right, (frame_width, frame_height))

    # crop the outer edges to remove monocular zones
    cropped_left, cropped_right = frame_rgb_left[:, edge_crop:], frame_rgb_right[:, -edge_crop:]
...
class StereoCamera(Camera):
    ...
    async def capture_frame(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        frame_rgb_left, frame_rgb_right, cropped_left, cropped_right = crop_stereo_image(frame=frame,
