...
def test_converting_to_video(tmp_path):
    ...
    rec_vid = Recorder(repo_id='test', root=root_2, single_arm=False, play_sound=False, task='test',
                       cams={'left': (480, 640, 3)}, use_video=True)
    rec.start_recording()
    ...
    for _ in range(NUM_EP):
        ...
        for _ in range(NUM_FRAMES):
            ...
            rec.add_observation(left_joints=left_joints,
                                ...
                                right_joints_target=right_joints_target,
                                cams={'observation.images.left': img})
            rec_vid.add_observation(left_joints=left_joints,
                                    right_joints=right_joints,
                                    left_joints_target=left_joints_target,
                                    right_joints_target=right_joints_target,
                                    cams={'observation.images.left': img})
    ...
    assert {f.name for f in root.iterdir() if f.name != 'images'} == {f.name for f in root_2.iterdir() if f.name != 'images'}
    ...
    assert len(parquet_rec) == len(parquet_rec_vid), f"Different number of parquet files: {len(parquet_rec)} vs {len(parquet_rec_vid)}"
