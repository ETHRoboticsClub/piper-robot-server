...
from ..config import END_EFFECTOR_LINK_NAME, JOINT_NAMES, NUM_JOINTS, URDF_TO_INTERNAL_NAME_MAP
...
class PyBulletVisualizer:
        ...
        for arm_name, robot_id in self.robot_ids.items():
            ...
            for urdf_name, internal_name in URDF_TO_INTERNAL_NAME_MAP.items():
                if internal_name in JOINT_NAMES and urdf_name in p_name_to_index:
                    ...
                    if getattr(logging, self.log_level.upper()) <= logging.INFO:
                        logger.info(
                            f"  Mapped: '{internal_name}' -> '{urdf_name}' (Index {p_name_to_index[urdf_name]})"
                        )
        ...
        for arm_name, robot_id in self.robot_ids.items():
            ...
            for i in range(num_joints):
                ...
                if link_name == END_EFFECTOR_LINK_NAME:
                    ...
                    if getattr(logging, self.log_level.upper()) <= logging.INFO:
                        logger.info(
                            f"Found end effector link '{END_EFFECTOR_LINK_NAME}' for {arm_name} robot at index {i}"
                        )
                    found = True
                    break
        ...
        for i in range(NUM_JOINTS):
            ...
            if pb_index is not None:
                ...
                if lower < upper:
                    ...
                    if getattr(logging, self.log_level.upper()) <= logging.INFO:
                        logger.info(
                            f"  {joint_name}: {self.joint_limits_min_deg[i]:.1f}° to {self.joint_limits_max_deg[i]:.1f}°"
                        )
                else:
                    if getattr(logging, self.log_level.upper()) <= logging.INFO:
    ...
    def _setup_camera(self):
        ...
        if getattr(logging, self.log_level.upper()) <= logging.INFO:
            logger.info(
                f"Camera positioned behind robot at distance={camera_distance}, yaw={camera_yaw}°, pitch={camera_pitch}°"
            )
    ...
    def update_marker_position(self, marker_name: str, position: np.ndarray, orientation: Optional[np.ndarray] = None):
        ...
        p.resetBasePositionAndOrientation(self.viz_markers[marker_name], position.tolist(), orientation)

    def update_coordinate_frame(
        self, frame_name: str, position: np.ndarray, orientation_quat: Optional[np.ndarray] = None
    ):
    ...
    def hide_frame(self, frame_name: str):
        ...
        if frame_name in self.viz_markers:
            ...
            for line_id in frame_lines:
                p.addUserDebugLine(
                    [0, 0, -1], [0, 0, -1], lineColorRGB=[0, 0, 0], lineWidth=1, replaceItemUniqueId=line_id
                )
