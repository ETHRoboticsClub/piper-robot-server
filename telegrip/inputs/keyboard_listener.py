...
from ..config import ANGLE_STEP, GRIPPER_STEP, POS_STEP, TelegripConfig
...
class KeyboardListener(BaseInputProvider):
    ...
    def print_controls(self):
        ...
        if getattr(logging, self.config.log_level.upper()) <= logging.INFO:
            ...
            logger.info("=" * 60)
            logger.info(
                f"Left arm position control: {'ACTIVE' if self.left_arm_state['position_control_active'] else 'INACTIVE'}"
            )
            logger.info(
                f"Right arm position control: {'ACTIVE' if self.right_arm_state['position_control_active'] else 'INACTIVE'}"
            )
        else:
    ...
    def _check_if_all_keys_released(self, arm: str):
        ...
        if (
            np.all(arm_state["delta_pos"] == 0)
            and arm_state["delta_wrist_roll"] == 0
            and arm_state["delta_wrist_flex"] == 0
        ):
    ...
    def _send_mode_change_goal(self, arm: str):
        ...
        goal = ControlGoal(arm=arm, mode=mode, metadata={"source": f"keyboard_mode_{arm}"})
        # Put goal in queue (non-blocking)
        try:
            self.command_queue.put_nowait(goal)
        except:
            ...
            try:
                ...
                for arm, arm_state in [("left", self.left_arm_state), ("right", self.right_arm_state)]:
                    if arm_state["position_control_active"]:
                        ...
                        if (
                            not arm_state["any_key_pressed"]
                            and arm_state["last_key_time"] > 0
                            and current_time - arm_state["last_key_time"] >= self.idle_timeout
                        ):
                        ...
                        if (
                            np.any(arm_state["delta_pos"] != 0)
                            or arm_state["delta_wrist_roll"] != 0
                            or arm_state["delta_wrist_flex"] != 0
                        ):
    ...
    def get_idle_status(self, arm: str) -> tuple:
        ...
        is_idle = not arm_state["any_key_pressed"] and arm_state["last_key_time"] > 0
