    ...
    def handle_config_post_request(self):
        ...
        try:
            ...
            if success:
                ...
                self.end_headers()
                self.wfile.write(
                    json.dumps({"success": True, "message": "Configuration updated successfully"}).encode("utf-8")
                )
    ...
    def handle_restart_request(self):
        ...
        try:
            if hasattr(self.server, "api_handler") and self.server.api_handler:
                ...
                self.end_headers()
                self.wfile.write(
                    json.dumps({"success": True, "message": "Teleoperation system restarted"}).encode("utf-8")
                )
            else:
                self.send_error(500, "System not available")
...
async def main():
    ...
    if log_level <= logging.INFO:
        ...
        logger.info(
            f"  Headless mode: {'enabled' if not config.enable_pybullet_gui and config.enable_pybullet else 'disabled'}"
        )
