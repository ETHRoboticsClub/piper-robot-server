window.LiveKitUtils = {
    asyncRoomConnect: async function (participantIdentity) {
        try {
          ...
          await window.LiveKitUtils.connectToRoom(participantIdentity);
        } catch (error) {
    ...
    connectToRoom: async function (participantIdentity, canPublish = false) {
        try {
        // Get token and LiveKit URL from Python backend
        const response = await this.getToken(participantIdentity, canPublish);
    ...
    getToken: async function (participantIdentity, canPublish = false) {
        try {
        const isVR =
            navigator.xr &&
            (await navigator.xr
        ...
        const authUrl = `https://${window.location.hostname}:${this.data.authServerPort}/api/get-token`;
